...
from _const._directory import camera_off_path_image
...
class KivyCamera(Image):
	...
	def clear_texture(self):
		self.texture = CoreImage(camera_off_path_image).texture

	@mainthread
	def on_start(self, model):
		if not self.camera_status:
			self.camera_status = self.STARTED
			self.main_screen.ids.on_off_btn.text = stop_webcam_text
			self.model = model

			if self.model is not None:
				if self.model.algorithm == algorithm_knn:
					self.algorithm = KNN_classifier(self.model, self.model.path_file_model)

				elif self.model.algorithm == algorithm_svm:
	...
	def on_stop(self):
		...
		self.main_screen.ids.on_off_btn.text = start_webcam_text
		if self._capture is not None:
			self.algorithm = None
			self.model = None
			self._capture.release()
	...
	def set_camera_port(self, camera_port):
		if (camera_port == port_0):
			self.camera_port = 0
		elif (camera_port == port_1):
			self.camera_port = 1
		elif (camera_port == port_2):
			self.camera_port = 2
		elif (camera_port == port_3):
			self.camera_port = 3
		elif (camera_port == port_4):
			self.camera_port = 4
