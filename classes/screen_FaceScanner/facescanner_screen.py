...
from screen_FaceScanner.kivy_camera import KivyCamera
...
from _const._const import camera_values, algorithm_knn, algorithm_svm, ALLOWED_EXTENSIONS
from _const._customization import text_unknown
from _const._directory import path_temp, path_file_stats, path_plt_facescreen_stats
...
class FaceScanner(Screen):
	...
	def __init__(self, **kw):
		...
		try:
			...
			if not os.path.exists(path_file_stats):
				open(path_file_stats, 'tw', encoding='utf-8').close()
	...
	def load_photo(self):
		...
		if photo_paths:
			...
			for img in photo_paths:
				...
				elif model.algorithm == algorithm_svm:
				...
				if is_loaded:
					...
					print(name)
					if (name.lower() == text_unknown.lower()):
	...
	def set_text_camera_spinner(self):
		self.camera_selected = camera_values[0]
	...
	def read_plot(self):
		data_csv = []
		if os.path.exists(path_file_stats):
			with open(path_file_stats, 'r') as csvfile:
				file_rd = csv.reader(csvfile, delimiter=',')
				for (i, row) in enumerate(file_rd):
					data_csv.append(row)
		else:
			self.clear_stats()
			return path_plt_facescreen_stats
		...
		if (len1):
			...
			ax.grid(axis='y')
			plt.savefig(path_plt_facescreen_stats)
			plt.clf()

			self.update_right_plot(path_plt_facescreen_stats)
		else:
			self.clear_stats()
	...
	def clear_stats(self):
		...
		with open(path_file_stats, "w", newline='') as gen_file:
			writing = csv.writer(gen_file, delimiter=',')
			for hour in range_hours:
	...
	def popup_photo(self):
		if (os.path.exists(path_plt_facescreen_stats)):
			try:
